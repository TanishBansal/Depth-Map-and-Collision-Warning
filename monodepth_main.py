...
def test(params):
	...
	disp_to_img = scipy.misc.imresize(disparities_pp[step].squeeze(), [256, 512])
	plt.imsave(os.path.join(output_directory, "image" + str(step)+ "_disp.png"), disp_to_img, cmap='plasma')
    #depth=0.54*721/(1240* disp_to_img)
    #plt.imsave(os.path.join(output_directory, "image" + str(step)+ "inv_disp.png"), depth, cmap='plasma')
